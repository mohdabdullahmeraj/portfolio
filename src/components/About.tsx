  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      mm.add("(max-width: 900px)", () => {
        ...
        gsap.set(imageRef.current, { opacity: 0, scale: 0.9, filter: "blur(20px)" });
        ...
        const scrollTl = gsap.timeline({
          scrollTrigger: {
            trigger: wrapperRef.current,
            start: "top top",
            end: "+=350%", // Extended pin for full sequential animation
            pin: innerRef.current,
            scrub: 1,
          },
        });

        // 1. Fade out left text
        scrollTl.to(leftTextRef.current, { filter: "blur(15px)", opacity: 0, scale: 0.9, duration: 1 }, 0);
        ...
        if (rightTextRef.current) {
          ...
          scrollTl.to(
            words,
            { filter: "blur(0px)", opacity: 1, y: 0, duration: 1, stagger: 0.05, ease: "power2.out" },
            0.8
          );

          // 3. Fade out right text
          scrollTl.to(
            words,
            { filter: "blur(15px)", opacity: 0, y: -20, duration: 0.8, stagger: 0.02, ease: "power2.in" },
            2.5
          );
        }

        // 4. Reveal image
        scrollTl.to(imageRef.current, { filter: "blur(0px)", opacity: 1, scale: 1, duration: 1.2, ease: "power2.out" }, 3.5);
      });
    }, wrapperRef);
